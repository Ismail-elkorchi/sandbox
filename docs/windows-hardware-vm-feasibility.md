...
| Image boot | HCS schema 2.1 can UEFI-boot a VHDX SCSI attachment as shown by the official quick start. The Windows VM owner must verify a signed descriptor plus kernel/bootloader, base VHDX, and guest-agent digests before calling HCS; no image is downloaded at runtime. Architecture-specific images are required. |
