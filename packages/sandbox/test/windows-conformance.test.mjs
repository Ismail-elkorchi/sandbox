  ...
  try {
    ...
    try {
      const result = await sandbox.run(options(executable, workspace, [
        "-e",
        "const p=require('child_process').spawn(process.execPath,['-e','setInterval(()=>{},1000)']);require('fs').writeFileSync(process.argv[2],String(p.pid));const n=require('net');const c=n.connect(+process.argv[1],'127.0.0.1');c.once('connect',()=>process.exit(91));c.once('error',()=>setInterval(()=>{},1000))",
        String(address.port),
        join(workspace, "pid"),
