...
impl Database {
    pub fn create(root: &Path, role: &str, schema: &str) -> Result<Self> {
        ...
        tx.execute_batch("PRAGMA application_id = 1397966385; PRAGMA user_version = 4; CREATE TABLE identity(role TEXT NOT NULL) STRICT;")?;
    ...
    pub fn open(root: &Path, role: &str) -> Result<Self> {
        ...
        if application != APPLICATION_ID || version != 4 {
